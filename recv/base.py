...
class AuthenticationDelegate(DefaultDelegate):
    ...
    def handleNotification(self, hnd, data):
        # Debug purposes
        print('Debug==>', 'data:', data, ' hnd:', hnd)
        if hnd == self.device._char_auth.getHandle(): # == 84
        ...
        elif hnd == self.device._char_heart_measure.getHandle(): # == 41
        ...
        # It can trigger the communication.
        elif hnd == self.device._char_fetch.getHandle(): # == 65
            if data[:3] == b'\x10\x01\x01':
         ...
         # from one given timestamp until now.
            ...
            print('self.device._char_activity.getHandle()', self.device._char_activity.getHandle())
            ...
            print("Unhandled Response " + hex(hnd) + ": " +
                                   str(data.encode("hex")) + " len:" + str(len(data)))
...
class MiBand2(Peripheral):
    ...
    def __init__(self, mac_address, timeout=0.5, debug=False):
        ...
        print('self._char_auth: ', self._char_auth)
    ...
    def _parse_raw_accel(self, byte):
        res = []
        for i in xrange(3):
            g = struct.unpack('hhh', byte[2 + i * 6:8 + i * 6])
            res.append({'x': g[0], 'y': g[1], 'wtf': g[2]})
        # WTF
        # if len(byte) == 20 and struct.unpack('b', byte[0])[0] == 2:
        #     print struct.unpack('B', byte[1])
        #     print "Accel x: %s y: %s z: %s" % struct.unpack('hhh', byte[2:8])
        #     print "Accel x: %s y: %s z: %s" % struct.unpack('hhh', byte[8:14])
        #     print "Accel x: %s y: %s z: %s" % struct.unpack('hhh', byte[14:])
        return res

    def _parse_raw_heart(self, byte):
        res = struct.unpack('HHHHHHH', byte[2:])
        return res

    def _parse_date(self, byte):
        # year = struct.unpack('h', byte[0:2])[0] if len(byte) >= 2 else None
        # month = struct.unpack('b', byte[2])[0] if len(byte) >= 3 else None
        # day = struct.unpack('b', byte[3])[0] if len(byte) >= 4 else None
        # hours = struct.unpack('b', byte[4])[0] if len(byte) >= 5 else None
        # minutes = struct.unpack('b', byte[5])[0] if len(byte) >= 6 else None
        # seconds = struct.unpack('b', byte[6])[0] if len(byte) >= 7 else None
        # day_of_week = struct.unpack('b', byte[7])[0] if len(byte) >= 8 else None
        # fractions256 = struct.unpack('b', byte[8])[0] if len(byte) >= 9 else None
        year = struct.unpack('h', byte[0:2])[0] if len(byte) >= 2 else None
        month = byte[2] if len(byte) >= 3 else None
        day = byte[3] if len(byte) >= 4 else None
        hours = byte[4] if len(byte) >= 5 else None
        minutes = byte[5] if len(byte) >= 6 else None
        seconds = byte[6] if len(byte) >= 7 else None
        day_of_week = byte[7] if len(byte) >= 8 else None
        fractions256 = byte[8] if len(byte) >= 9 else None

        return {"date": datetime(*(year, month, day, hours, minutes, seconds)), "day_of_week": day_of_week, "fractions256": fractions256}

    def _parse_battery_response(self, byte):
        # level = struct.unpack('b', byte[1])[0] if len(byte) >= 2 else None
        # last_level = struct.unpack('b', byte[19])[0] if len(byte) >= 20 else None
        # status = 'normal' if struct.unpack('b', byte[2])[0] == 0 else "charging"
        level = byte[1] if len(byte) >= 2 else None
        last_level = byte[19] if len(byte) >= 20 else None
        status = 'normal' if byte[2] == 0 else "charging"
        datetime_last_charge = self._parse_date(byte[11:18])
        datetime_last_off = self._parse_date(byte[3:10])
    ...
    def _parse_queue(self):
        while True:
            try:
                res = self.queue.get(False)
                _type = res[0]
                print('Debug==> res:', res, '_type:',_type)
    ...
    def send_alert(self, _type):
        print('debug: (send alert) _type:', _type)
    ...
    def start_raw_data_realtime(self, heart_measure_callback=None, heart_raw_callback=None, accel_raw_callback=None):
        ...
        char_d = char_m.getDescriptors(forUUID=UUIDS.NOTIFICATION_DESCRIPTOR)[0]
        char_ctrl = self.svc_heart.getCharacteristics(UUIDS.CHARACTERISTIC_HEART_RATE_CONTROL)[0]
        #print('Debug raw data real time: ', char_m, char_d, char_ctrl)
    ...
    def start_get_previews_data(self, start_timestamp):
        ...
        ts = year + bytes(month) + bytes(day) + bytes(hour) + bytes(minute)
