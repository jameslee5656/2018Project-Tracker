...
var POI,POIs1 = new Array(), POIs2 = new Array(), POIs3 = new Array();
...
var myVar=setInterval(function(){changeRanking()},30000);
...
function getPOI(rank,lng,lat){
  ...
  request.onload = function() {
    ...
    if(recommendName in recommend_name || !(POI.results[recommendResult].photos[0].photo_reference)){
      while(recommendName in recommend_name || !(POI.results[recommendResult].photos[0].photo_reference)){
        recommendResult++;
        recommendName = POI.results[recommendResult].name;
      }
    ...
    POIs2[rank] = POI_point;
    POIs3[rank] = POI_point;
  }
}
...
function changePOI_mode(mode){
  ...
  if(mode != currentMode){
    ...
    var arrayname = 'POIs' + mode;
    for(var i =1; i < 11; i++){
      document.getElementById("R-name"+i).innerHTML = arrayname[i].name;
      document.getElementById("R-address"+i).innerHTML = arrayname[i].address;
      document.getElementById("R-pic"+i).style.backgroundImage = "url('"+arrayname[i].photo+"')";
      document.getElementById("R-dis"+i).innerHTML = '距離' + arrayname[i].distance + '公尺';
      createRoute(i,arrayname[i].lat,arrayname[i].lng);
      var PointStart ={
        lat: current_user_lat,
        lng: current_user_lng
      }
...
function modePrechange(){
  for(var i = 1; i < 10; i++){
    console.log(POIs3[i].distance);
    if(POIs3[i].distance > POIs3[i+1].distance){
      var tempValue = POIs3[i].distance;
      POIs3[i].distance = POIs3[i+1].distance;
      POIs3[i+1].distance = tempValue;
    }
  }
}
//show direction
